...
This writes `.devcontainer/devcontainer.json` from the vendored template, setting `name` to the repo directory name. It refuses to overwrite an existing file unless you pass `--force`.

Then open the project in VS Code and **Reopen in Container**.

### Copy-paste fallback

Copy [`templates/devcontainer.json`](templates/devcontainer.json) into the project as `.devcontainer/devcontainer.json` and replace `<project-name>` with the project's name.
